...
class TestProvSet(unittest.TestCase):
    ...
    def test_get_line_len(self):
        with open(self.file_path, 'rb') as test_file:
            line_len = ProvSet._get_line_len(test_file)
            self.assertIsNotNone(line_len)
            self.assertEqual(line_len, ProvSet.initial_line_len)
            self.assertEqual(test_file.tell(), 0)

    def test_increase_line_len(self):
        increment = 1
        result = ProvSet._increase_line_len(self.file_path, ProvSet.initial_line_len + increment)
        self.assertIsNone(result)
        with open(self.file_path, 'rt', encoding='ascii') as test_file:
            for line in test_file:
                self.assertEqual(len(line), ProvSet.initial_line_len + increment)

        self.assertRaises(ValueError, ProvSet._increase_line_len, self.file_path, -1)
        self.assertRaises(ValueError, ProvSet._increase_line_len, self.file_path, ProvSet.initial_line_len - 1)

    def test_is_a_valid_line(self):
        with self.subTest("line is 'abc \\n'"):
            line = 'abc \n'.encode('ascii')
            result = ProvSet._is_a_valid_line(line)
    ...
    def test_fix_previous_lines(self):
        with open(self.file_path, "wb") as test_file:
            ...
            for i in range(0, num_lines):
                line = '\0' * ProvSet.initial_line_len
                test_file.write(line.encode('ascii'))
            last_line = '1'.ljust(ProvSet.initial_line_len - 1, ProvSet.trailing_char) + '\n'
            test_file.write(last_line.encode('ascii'))

        with open(self.file_path, 'r+b') as test_file:
            test_file.seek(ProvSet.initial_line_len * num_lines)
            result = ProvSet._fix_previous_lines(test_file, ProvSet.initial_line_len)
            self.assertIsNone(result)
    ...
    def test_read_number(self):
        ...
        with open(self.file_path, 'r+b') as test_file:
            num_of_line = 35
            test_file.seek(ProvSet.initial_line_len * (num_of_line - 1))
            line = str(number).ljust(ProvSet.initial_line_len - 1, ProvSet.trailing_char) + '\n'
            test_file.write(line.encode('ascii'))

        read_number, line_len = ProvSet._read_number(self.file_path, num_of_line)
        self.assertIsNotNone(read_number)
        self.assertIsNotNone(line_len)
        self.assertEqual(read_number, number)
        self.assertEqual(line_len, ProvSet.initial_line_len)

        self.assertRaises(ValueError, ProvSet._read_number, self.file_path, -1)
    ...
    def test_add_number(self):
        ...
        read_number = ProvSet._add_number(self.file_path, num_of_line)
        self.assertIsNotNone(read_number)
