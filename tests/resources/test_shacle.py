    ...
    def test_reader(self):
        ...
        for triple in g:
            ...
            if isinstance(o, Literal) and o.datatype:
                results.append(
                    {
                        "s": {"type": "uri", "value": str(triple[0])},
                        "p": {"type": "uri", "value": str(triple[1])},
                        "o": {"type": o_type, "value": str(triple[2]), "datatype": str(o.datatype)},
                    }
                )
            else:
                results.append(
                    {
                        "s": {"type": "uri", "value": str(triple[0])},
                        "p": {"type": "uri", "value": str(triple[1])},
                        "o": {"type": o_type, "value": str(triple[2])},
                    }
                )
        reader.import_entities_from_graph(
            g_set, results, resp_agent="https://orcid.org/0000-0002-8420-0696", enable_validation=True, closed=False
        )
        self.assertEqual(
            set(str(s) for s in g_set.res_to_entity.keys()),
            {
